...
class SingleModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        if opt.vgg > 0:      # 如果使用VGG，读取VGG模型和VGG损失
            self.vgg_loss = networks.PerceptualLoss(opt)         # VGG 全局损失
            if self.opt.IN_vgg:
                self.vgg_patch_loss = networks.PerceptualLoss(opt)  # VGG patch损失
        ...
        elif opt.fcn > 0:    # 同上，如果使用FCN，读取FCN模型和FCN损失
        ...
        if not self.isTrain or opt.continue_train:     # 如果不是训练或者继续训练，加载已经训练好的权重
